...
*iefolder will not work properly if the line above is edited
   ...
   *Encrypted round sub-folder globals
   global mdl_encrypt            "$encryptFolder/Round Midline Encrypted" 
   global mdl_dtRaw              "$mdl_encrypt/Raw Identified Data" 
   global mdl_doImp              "$mdl_encrypt/Dofiles Import" 
   global mdl_HFC                "$mdl_encrypt/High Frequency Checks" 

   *DataSets sub-folder globals
   global mdl_dt                 "$mdl/DataSets" 
   global mdl_dtDeID             "$mdl_dt/Deidentified" 
   global mdl_dtInt              "$mdl_dt/Intermediate" 
   global mdl_dtFin              "$mdl_dt/Final" 

   *Dofile sub-folder globals
   global mdl_do                 "$mdl/Dofiles" 
   global mdl_doCln              "$mdl_do/Cleaning" 
   global mdl_doCon              "$mdl_do/Construct" 
   global mdl_doAnl              "$mdl_do/Analysis" 

   *Output sub-folder globals
   global mdl_out                "$mdl/Output" 
   global mdl_outRaw             "$mdl_out/Raw" 
   global mdl_outFin             "$mdl_out/Final" 

   *Questionnaire sub-folder globals
   global mdl_prld               "$mdl_quest/PreloadData" 
   global mdl_doc                "$mdl_quest/Questionnaire Documentation" 
